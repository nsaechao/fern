...
export async function generateIntermediateRepresentation({
    ...
    for (const [name, definition] of Object.entries(workspace.definition.rootApiFile.contents.environments ?? {})) {
        irGraph.addEnvironment(name);
        if (typeof definition !== "string" && definition.audiences != null) {
            irGraph.markEnvironmentForAudience(name, definition.audiences);
        }
    }
...
function filterIntermediateRepresentationForAudiences(
    ...
    let filteredEnvironments: Environments | undefined = undefined;
    if (intermediateRepresentation.environments != null) {
        filteredEnvironments = intermediateRepresentation.environments.environments._visit<Environments | undefined>({
            singleBaseUrl: (singleUrlEnvs) => {
                return Environments.singleBaseUrl({
                    environments: singleUrlEnvs.environments.filter((env) => filteredIr.hasEnvironment(env.id))
                });
            },
            multipleBaseUrls: (multibleBaseUrlEnvs) => {
                return Environments.multipleBaseUrls({
                    baseUrls: multibleBaseUrlEnvs.baseUrls,
                    environments: multibleBaseUrlEnvs.environments.filter((env) => filteredIr.hasEnvironment(env.id))
                });
            },
            _other: () => undefined
        });
    ...
    return {
        ...intermediateRepresentation,
        environments:
            filteredEnvironments != null
                ? {
                      defaultEnvironment:
                          intermediateRepresentation.environments?.defaultEnvironment != null &&
                          filteredIr.hasEnvironment(intermediateRepresentation.environments?.defaultEnvironment)
                              ? intermediateRepresentation.environments?.defaultEnvironment
                              : undefined,
